...
| ORM | `prisma` / `@prisma/client` | **7.10.0** | Prisma 8 ainda é RC. Prisma 7 exige *driver adapter*: `@prisma/adapter-pg` 7.10.0 + `pg` 8.23.0 e `prisma.config.ts` |
...
**Sem dependência para:** rate limit (token bucket em memória, com interface para trocar por Redis), validação de MIME real (leitura de *magic bytes* dos 5 tipos aceitos: PDF, XML, PNG, JPEG, XLSX/CSV), validação de CNPJ (dígito verificador implementado em `/lib/validation/cnpj.ts`), máscaras de moeda (formatação via `Intl.NumberFormat('pt-BR')`).
...
- **P1** Dados da empresa prestadora (nome, CNPJ, logo) vêm de variáveis `COMPANY_*` no `.env` e de `/public/logo.png`; editáveis por Admin em *Configurações* a partir da Fase 6. No seed: "Prestadora Demo Ltda".
